...
---------------
required --->
[root@ip-172-31-32-200 jagadeesh]# ll
total 12
-rw-r--r--. 1 root root 1115 Oct 27 02:39 jagadeesh.crt
-rw-r--r--. 1 root root  915 Oct 27 02:37 jagadeesh.csr
-rw-------. 1 root root 1704 Oct 27 02:32 jagadeesh.key
[root@ip-172-31-32-200 jagadeesh]#

1)lets create a folder -->mkdir jagadeesh && cd jagadeesh
2)Generate a key using openssl -->openssl genrsa -out jagadeesh.key 2048
3)Generate a Client Sign Request (CSR) :----> openssl req -new -key jagadeesh.key -out jagadeesh.csr -subj "/CN=jagadeesh/O=group1"
4)Generate the certificate (CRT):--->openssl x509 -req -in jagadeesh.csr -CA ~/.minikube/ca.crt -CAkey ~/.minikube/ca.key -CAcreateserial -out jagadeesh.crt -days 500

Steps to create user.

lets create a user:--->kubectl config set-credentials jagadeesh --client-certificate=jagadeesh.crt --client-key=jagadeesh.key
Set a context entry in kubeconfig:----> kubectl config set-context my-context --cluster=minikube --user=jagadeesh
To see the user:----->  kubectl config view
Switch to user:---->  kubectl config use-context my-context
